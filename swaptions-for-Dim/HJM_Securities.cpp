...
#else
{
  ...
  FTYPE pdSwaptionPrice[2];
    #ifdef SETAFFIN
    if (cpustride != -1) {
        //std::cout << "Setting affinity of thread " << tid 
        //    << " to CPU number " << tid+cpustride << std::endl;
        cpu_set_t cpumask; 
        CPU_ZERO(&cpumask);
        int cpu = tid+cpustride;
        CPU_SET(cpu,&cpumask);
        //int res = sched_setaffinity(0,sizeof(cpu_set_t),&cpumask);
        pthread_t current_thread = pthread_self();
        int res = pthread_setaffinity_np(current_thread,sizeof(cpu_set_t),&cpumask);
        if (res != 0) {
          perror("Error in setting thread affinity");
          exit(1);
        }
    } else ;//std::cout << "Affinity not set. No valid cpustride found" << std::endl;
    #endif
...
//	printf("PARSEC Benchmark Suite\n");
    ...
    if (cpustride == -1) {
        int fd = -1;
        char stride[10];
        if ((fd = open("affinity.txt",O_RDONLY)) > 0) {
       //     std::cout << "File descriptor: " << fd << std::endl;
            int res; 
            if ((res = read(fd,&stride,10)) > 0) {
                cpustride = atoi(stride);
//                std::cout << "Thread binding starting in CPU " << cpustride << std::endl;
            }
            close(fd);
        }
    }
    if (affinitystride <=7 && affinitystride >= 0) cpustride = affinitystride;
#endif
